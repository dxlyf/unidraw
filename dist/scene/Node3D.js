...
export class Node3D {
     ...
     */
    updateWorldMatrix(force = false, parentWorld, parentWorldVersion = -1) {
        const localChanged = this._updateLocal();
        const worldChanged = force || localChanged || this._lastParentVersion !== parentWorldVersion;
        if (worldChanged) {
            if (parentWorld)
                Mat4.multiply(parentWorld, this.matrix, this.worldMatrix);
            else
