...
export class App {
    ...
    stop() {
        this._running = false;
        this._lastNow = 0;
        if (typeof cancelAnimationFrame === "function" && this._rafId)
    ...
    _step(dt) {
        ...
        for (let i = 0; i < this.plugins.length; i++)
            this.plugins[i].update?.(this._ctx, step, this.stats.time);
        this.mixer.update(step);
        this.tweens.update(step);
        // 3) 渲染通道
        //    阴影贴图必须在主 pass 之前提交（WebGPU 禁止同一 submit 内既写又读同一张纹理）
        if (this.shadows)
            this.shadows.renderAndSubmit(this.scene, this.camera, this.sceneRenderer, step);
        const pass = this.renderer.beginFrame();
        try {
            for (let i = 0; i < this.plugins.length; i++)
                this.plugins[i].beforeRender?.(this._ctx, pass);
            ...
            for (const cb of this._frameCallbacks)
                cb(pass, step, this.stats.time);
            for (let i = 0; i < this.plugins.length; i++)
                this.plugins[i].afterRender?.(this._ctx, pass);
        }
        finally {
        ...
        if (this.canvas && this._picker)
            this._picker.resize(this.canvas.width, this.canvas.height);
    }
    /** 便捷：设置清屏色 */
    setClear(color) {
    ...
    dispose() {
        ...
        for (let i = this.plugins.length - 1; i >= 0; i--)
            this.plugins[i].dispose?.(this._ctx);
        this.plugins.length = 0;
        this._frameCallbacks.clear();
        this.mixer.dispose();
        this.tweens.stopAll();
        if (this._ownsShadows)
    ...
    _resizeNotify() {
        for (let i = 0; i < this.plugins.length; i++)
            this.plugins[i].resize?.(this._ctx, this.stats.width, this.stats.height);
    }
}
