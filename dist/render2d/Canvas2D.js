...
import { fillTriangles } from "./fill.js";
...
import { sourceSize, textureFromImageSource } from "../render/texture/image.js";
...
const SOLID_PAINT = { kind: 0, frame: [0, 0, 0, 0, 0, 0, 1], lut: null, vcolor: [1, 1, 1, 1] };
export class Canvas2D {
     ...
     */
    flatPipelines = new Map();
    texPipelines = new Map();
    sampler;
    textureGroups = new WeakMap();
    pipelineFormat = null;
    ...
    layerSamples = 0;
    blendModePass = null;
    layerPresentPass = null;
    layerPassKey = "";
    layerPresentKey = "";
    /** 图案采样器（按重复方式缓存） */
    patternSamplers = new Map();
    viewW = 1;
    viewH = 1;
    ...
    pipelineFor(kind, sampleCount, comp = "source-over") {
        ...
        if (pipeline)
            return pipeline;
        const format = (this.pipelineFormat ?? this.device.canvasFormat() ?? "rgba8unorm");
                    ...
                    kind === "flat"
                        ? {
                            ...
                            attributes: [
                                ...
                                { location: 3, format: "float32x4", offset: 32 },
                                { location: 4, format: "float32x4", offset: 48 },
                            ],
                        }
                        : {
                            arrayStride: 32,
    ...
    gradientLut(kind, stops) {
        ...
        if (!ctx)
            throw new Error("[unidraw] 无法创建 2D 画布用于渐变 LUT");
        ...
        if (this.gradLuts.size > GRADIENT_LUT_CACHE) {
            const oldest = this.gradLuts.keys().next().value;
            if (oldest !== undefined) {
    ...
    resolvePaint(style) {
        ...
        if (style instanceof CanvasPattern) {
            ...
            const repFlag = (style.repetition === "repeat" || style.repetition === "repeat-x" ? 1 : 0) +
    ...
    set globalCompositeOperation(v) {
        if (!blendForComposite(v) && dstTextureBlendIndex(v) < 0) {
            if (!this.warnedComposite.has(v)) {
                ...
                logger.warn(`[unidraw] Canvas2D 暂不支持 globalCompositeOperation = "${v}"，已回退为 source-over。` +
    ...
    restore() {
        ...
        if (!s)
            throw new Error("[unidraw] Canvas2D.restore() 与 save() 不匹配");
        this.state = { ...s };
    }
    translate(tx, ty) {
    ...
    flush(pass, viewProj) {
        ...
        for (const op of this.ops)
            if (op.shadow !== undefined && !shadowKeys.includes(op.shadow))
                shadowKeys.push(op.shadow);
        ...
        for (let i = 0; i < this.ops.length; i++) {
            const o = this.ops[i];
            if (o.shadow === undefined && dstTextureBlendIndex(o.comp) >= 0)
                lastBlendOp = i;
        }
        ...
        if (needsLayer) {
            ...
            if (this.layerSamples !== samples) {
                for (const t of [this.layerA, this.layerB, this.layerSrc])
                    t?.dispose();
                this.layerA = null;
                this.layerB = null;
                this.layerSrc = null;
        ...
        for (let opIndex = 0; opIndex < this.ops.length; opIndex++) {
            ...
            if (!clipInit || !sameClip(lastClip, c)) {
                if (c)
                    sink.setScissorRect(c.x, c.y, c.w, c.h);
                else
                    sink.setScissorRect(0, 0, this.viewW, this.viewH);
                lastClip = c;
                clipInit = true;
            }
            ...
            const wantTex = (op.kind === "flat" ? (op.lut ?? this.solidLut) : op.texture);
            ...
            if (n > 0)
                sink.drawIndexed(n, 1, op.iStart, 0, 0);
        }
        ...
        if (this.shadowLayers.size > SHADOW_LAYER_CACHE)
            this.pruneShadowLayers();
    }
    /** 图层模式用的颜色格式（与调用方 pass 一致） */
    layerFormat() {
        return (this.pipelineFormat ?? this.device.canvasFormat() ?? "rgba8unorm");
    }
    /** 采样的是几何渲染出来的纹理，WebGPU 的行序反过来（与 `CopyPass.flipY` 同一套道理） */
    layerFlipY() {
        return this.device.kind === "webgpu";
    }
    ensureLayerTarget(which, w, h, format, sampleCount) {
    ...
    ensureBlendModePass(format, sampleCount) {
    ...
    ensureLayerPresentPass(format, sampleCount) {
        const key = `${format}|${sampleCount}`;
     ...
     */
    drawOpRaw(pass, op, sampleCount) {
        const c = op.clip;
        if (c)
            pass.setScissorRect(c.x, c.y, c.w, c.h);
        else
            pass.setScissorRect(0, 0, this.viewW, this.viewH);
        if (op.kind === "flat") {
    ...
    textureGroup(texture, sampler = this.sampler) {
    ...
    pushFlat(ux, uy) {
        ...
        const f = paint.frame;
        this.flatV.push(p.x, p.y, c[0], c[1], c[2], c[3], ux, uy, paint.kind, f[0], f[1], f[2], f[3], f[4], f[5], f[6]);
        return v;
    }
    pushTextV(ux, uy, u, v, style) {
    ...
    /** 文字四边形（字形图集遮罩 × 顶点色） */
    pushTextQuad(left, top, w, h, style) {
    ...
    recordFlat(iStart, polys) {
        ...
        if (!polys || !blendForComposite(this.state.globalCompositeOperation)?.clearsOutside)
            return;
        const devPolys = polys.map((contour) => contour.map(([x, y]) => {
    ...
    shadowColorKey = "";
    shadowColorValue = { r: 0, g: 0, b: 0, a: 0 };
    /**
    ...
    emitDilation(polys, radius) {
        ...
        const disc = (cx, cy) => {
            ...
            for (let i = 0; i <= SEG; i++) {
                ...
                if (prev >= 0)
                    this.pushTri("flat", center, prev, v);
                prev = v;
            }
        };
    ...
    emitShadowTris(tris, polys) {
        if (!this.shadowEnabled() || tris.length === 0)
            return;
        ...
        for (const tri of tris) {
            const ids = [this.pushFlat(tri[0][0], tri[0][1]), this.pushFlat(tri[1][0], tri[1][1]), this.pushFlat(tri[2][0], tri[2][1])];
            this.pushTri("flat", ids[0], ids[1], ids[2]);
        }
        // 扩散：**几何**向外扩张（轮廓各边外扩成四边形 + 每个顶点画圆盘，并集即
        // 「多边形 ⊕ 半径 spread 的圆」），完全不额外增加 pass，也不靠模糊去凑。
        if (this.state.shadowSpread > 0 && polys)
            this.emitDilation(polys, this.state.shadowSpread);
        this.state.ctm = base;
        this.ops.push({
            kind: "flat",
            clip: this.state.clip ? { ...this.state.clip } : null,
    ...
    emitShadowText(left, top, w, h, texture) {
    ...
    drawShadowOps(pass, groupKey, sampleCount, scale = 1) {
        let lastKind = null;
        let lastTex = null;
        for (const op of this.ops) {
            if (op.shadow !== groupKey)
                continue;
            // 裁剪矩形是全尺寸设备像素，遮罩缩小后要一起缩（scissor 永远是附件的物理像素）
            const c = op.clip;
            if (c)
                pass.setScissorRect(Math.round(c.x / scale), Math.round(c.y / scale), Math.max(1, Math.round(c.w / scale)), Math.max(1, Math.round(c.h / scale)));
            else
                pass.setScissorRect(0, 0, Math.max(1, Math.floor(this.viewW / scale)), Math.max(1, Math.floor(this.viewH / scale)));
            if (lastKind !== op.kind) {
                lastKind = op.kind;
                if (op.kind === "flat") {
                    pass.setPipeline(this.pipelineFor("flat", sampleCount, "source-over"));
            ...
            const wantTex = (op.kind === "flat" ? this.solidLut : op.texture);
            if (lastTex !== wantTex) {
    ...
    renderShadowLayer(groupKey, sampleCount) {
        ...
        // 必须每组一份（同上面的理由）。
        if (!this.shadowBlurPass)
            this.shadowBlurPass = new BlurPass(this.device, format);
        ...
        if (layer.dst)
            layer.dst.resize(bw, bh);
        else
            layer.dst = new RenderTarget(this.device, { label: "2d-shadow-dst", width: bw, height: bh, format, depth: false, sampleCount: 1 });
        const dst = layer.dst;
        const iterations = 1;
        const step = radius / scale;
        let blurSrc = mask.texture;
     ...
     * **参数恒定**的直通拷贝。
     */
    tintShadowLayer(layer, source, w, h, group) {
        const format = (this.pipelineFormat ?? this.device.canvasFormat() ?? "rgba8unorm");
        if (!this.shadowBlurPass)
            this.shadowBlurPass = new BlurPass(this.device, format);
        if (layer.tinted)
            layer.tinted.resize(w, h);
        else
            layer.tinted = new RenderTarget(this.device, { label: "2d-shadow-tinted", width: w, height: h, format, depth: false, sampleCount: 1 });
    ...
    ensureShadowTmp(w, h, format) {
        if (this.shadowTmp)
            this.shadowTmp.resize(w, h);
        else
            this.shadowTmp = new RenderTarget(this.device, { label: "2d-shadow-tmp", width: w, height: h, format, depth: false, sampleCount: 1 });
    ...
    pruneShadowLayers() {
        for (const [key, layer] of this.shadowLayers) {
            if (this.shadowLayers.size <= SHADOW_LAYER_CACHE)
                break;
            if (this.shadowLayersUsed.has(key))
                continue;
            layer.mask.dispose();
            layer.dst?.dispose();
    ...
    stroke() {
        ...
        for (const ct of raw) {
            ...
            for (let i = 0; i < src.length; i++) {
                ...
                if (!b || Math.hypot(a[0] - b[0], a[1] - b[1]) > 1e-4)
                    out.push(a);
            }
        ...
        for (const c of contours)
            if (c.points.length >= 3)
                strokePolys.push(c.points);
        const quadRaw = (ax, ay, bx, by, cx, cy, dx, dy) => {
            const p = (x, y) => this.pushFlat(x, y);
            const v0 = p(ax, ay);
            const v1 = p(bx, by);
            const v2 = p(cx, cy);
            const v3 = p(dx, dy);
        ...
        // 单段矩形
        const segmentQuad = (ax, ay, bx, by, nx, ny) => {
            quadRaw(ax + nx, ay + ny, bx + nx, by + ny, bx - nx, by - ny, ax - nx, ay - ny);
        };
        const fan = (cx, cy, a0, a1) => {
            ...
            for (let i = 0; i <= n; i++) {
                ...
                if (prev >= 0)
                    this.pushTri("flat", vc, prev, v);
                prev = v;
            }
        };
        const cap = (px, py, dirx, diry, nxx, nyy) => {
        ...
        const bodyHw = hw;
        if (this.shadowEnabled() && this.state.shadowSpread > 0)
            hw = bodyHw + this.state.shadowSpread;
    ...
    joinCorner(p0, p1, p2, hw) {
        ...
        const outer = (ex, ey, inv) => (s > 0 ? { x: (ey / inv) * hw, y: (-ex / inv) * hw } : { x: (-ey / inv) * hw, y: (ex / inv) * hw });
    ...
    clipRect(x, y, w, h) {
        ...
        const rect = { x: Math.round(minX), y: Math.round(minY), w: Math.round(maxX - minX), h: Math.round(maxY - minY) };
     ...
     */
    textureFor(source) {
        const cached = this.imageTextures.get(source);
        if (cached)
            return cached;
        const tex = textureFromImageSource(this.device, source, { label: "2d-image" });
        this.imageTextures.set(source, tex);
        return tex;
    }
    /** 让图像源的缓存纹理失效（源内容变了要重新上传时调用） */
    invalidateImage(source) {
        const tex = this.imageTextures.get(source);
        if (!tex)
            return;
        tex.destroy();
        this.imageTextures.delete(source);
    }
    /**
     * 绘制图像（三种重载与原生一致）：
    ...
    drawText(text, x, y, maxWidth, strokeWidth) {
        if (!text)
            return this;
        ...
        const baselineOffset = baseline === "top" || baseline === "hanging"
