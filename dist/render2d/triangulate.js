...
export function polygonArea2(pts) {
    let s = 0;
    const n = pts.length;
    for (let i = 0; i < n; i++) {
        const a = pts[i];
...
/** 用 ±1 统一绕序后的“左侧”判定 */
function pointInTri(px, py, ax, ay, bx, by, cx, cy, sign) {
 ...
 */
export function triangulateSimplePolygon(pts) {
    const n = pts.length;
    if (n < 3)
        return null;
    ...
    while (ring.length > 3) {
        ...
        for (let k = 0; k < m && !clipped; k++) {
            ...
            const b = pts[i];
            const c = pts[inx];
            if (sign * cross2(a[0], a[1], b[0], b[1], c[0], c[1]) < 1e-9)
                continue; // 非凸耳
            let empty = true;
            for (const j of ring) {
                if (j === ip || j === i || j === inx)
