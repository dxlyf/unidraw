...
export class Path2D {
    ...
    roundRect(x, y, w, h, r) {
        ...
        const radii = rr.map((v) => Math.max(0, Math.min(v, Math.min(w, h) / 2)));
    ...
    flatten(tolerance = 0.25) {
        ...
        let px = 0;
        let py = 0;
        const closeCurrent = (closed) => {
            if (cur && cur.length > 0)
                contours.push({ points: cur, closed });
            cur = null;
        };
        const ensure = () => {
            if (!cur) {
    ...
    /** 自适应细分二次贝塞尔 */
    flattenQuadTo(ax, ay, cx, cy, bx, by, tol, depth, out) {
    ...
    flattenCubicTo(ax, ay, c1x, c1y, c2x, c2y, bx, by, tol, depth, out) {
        if (depth > 18) {
            out.push([bx, by]);
            return;
        }
    ...
    arcToPoints(x0, y0, x1, y1, x2, y2, radius, tol) {
        const d01 = Math.hypot(x1 - x0, y1 - y0);
        const d12 = Math.hypot(x2 - x1, y2 - y1);
        if (d01 < 1e-9 || d12 < 1e-9 || radius <= 0)
            return [[x1, y1]];
        const ax = (x0 - x1) / d01;
        const ay = (y0 - y1) / d01;
        const bx = (x2 - x1) / d12;
        ...
        if (delta < 1e-6 || delta > Math.PI - 1e-3)
            return [[x1, y1]]; // 近似直线
        // 半径超过某条腿的承载能力时收紧
