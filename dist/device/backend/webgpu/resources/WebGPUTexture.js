...
export class WebGPUTexture extends Texture {
    ...
    constructor(device, desc) {
        ...
        if (sampleCount > 1) {
            ...
            assert((desc.usage & (TextureUsage.TEXTURE_BINDING | TextureUsage.STORAGE_BINDING | TextureUsage.COPY_SRC)) === 0, "多采样纹理不能采样/拷贝，只能作为渲染附件（渲染结果请用 resolveTo 解析到普通纹理）");
        }
    ...
    upload(data, options = {}) {
        ...
        this._device.gpu.queue.writeTexture({ texture: this.gpuTexture, mipLevel: options.mipLevel ?? 0, origin: { x, y, z } }, payload, { bytesPerRow: aligned, rowsPerImage }, { width, height, depthOrArrayLayers: depthLayers });
    }
    generateMipmaps() {
