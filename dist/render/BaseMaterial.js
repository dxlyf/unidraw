...
import { CAMERA_FIELDS, INSTANCE_VERTEX_SLOT, MATERIAL_FIELDS, MODEL_FIELDS, STANDARD_VERTEX_STATE, STANDARD_VERTEX_STATE_INSTANCED, defaultBlendState, defaultGroupEntries, depthFormatOf, targetFormatOf, } from "./materialCommon.js";
...
export class BaseMaterial {
    ...
    _instancedPipelineFor(sampleCount) {
        ...
        if (cached)
            return cached;
        const pipeline = this._createPipeline(sampleCount, targetFormatOf(this.device, this._opts), this._opts.depth !== false, true, program);
        this._instancedPipelines.set(sampleCount, pipeline);
        return pipeline;
    }
    _createPipeline(sampleCount, targetFormat, depth, instanced, program = this._program) {
     ...
     */
    _resolveInstancedProgram() {
        if (this._instancedProgram !== undefined)
    ...
    constructor(device, program, opts, extraLayoutEntries = []) {
        this.device = device;
        ...
        this._unhookFlush = device.onBeforeSubmit(() => {
            for (let i = 0; i < this._flushBlocks.length; i++)
                this._flushBlocks[i].flushPending();
        });
    }
    /**
     ...
     */
    beginFrame(viewProjection, cameraPos, lights) {
    ...
    setMaterialParams(x = 0, y = 0, z = 0, w = 0) {
     ...
     */
    drawInstanced(pass, geometry, model, source) {
    ...
    _recordDrawState(pass, geometry, model, pipeline, instanceBuffer) {
        ...
        if (this._modelSlotCursor >= this._modelSlotCount)
            this.growModelRing(this._modelSlotCursor + 1);
        ...
        for (let i = 0; i < extra.length; i++)
            offsets.push(extra[i]);
        // 管线/绑定组状态由后端做冗余消除；这里始终记录，避免多材质交替时状态错乱
        pass.setPipeline(pipeline);
        pass.setBindGroup(0, this.bindGroup, offsets);
        pass.setVertexBuffer(0, geometry.vertexBuffer);
        if (instanceBuffer)
            pass.setVertexBuffer(INSTANCE_VERTEX_SLOT, instanceBuffer);
        return slot;
    }
    /** 绘制任意几何体 + 模型矩阵。 */
    drawGeometry(pass, geometry, model) {
