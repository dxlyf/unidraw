...
import { shadowResources } from "./ShadowResources.js";
import { MAX_SHADOW_MAPS } from "./constants.js";
import { SHADOW_KIND_DIRECTIONAL, SHADOW_KIND_SPOT } from "./ShadowState.js";
import { shadowFilterCode } from "./ShadowSettings.js";
export class ShadowRenderer {
    device;
    resources;
    stats = { maps: 0, drawn: 0, skipped: 0, fitRadius: 0, texelWorld: 0, biasDepth: 0 };
    ...
    /** 拟合平滑时间常数（秒） */
    stabilizeTau;
    _camera = new ShadowCamera();
    _lights = [];
    _center = new Vec3();
    _targetCenter = new Vec3();
    _fitCenter = new Vec3();
    _worldPos = new Vec3();
    _radius = 0;
    _fitRadius = 0;
    ...
    constructor(device, options = {}) {
        ...
        if (options.material)
            this._materials.set("0:back", options.material);
     ...
     */
    render(encoder, scene, camera, sceneRenderer, dt = 1 / 60) {
        this.stats.maps = 0;
        this.stats.drawn = 0;
        ...
        for (let i = 0; i < lights.length; i++) {
            ...
            if (directional) {
                ...
                this._camera.fitDirectional(light.direction, this._fitCenter, radius, map.size, settings.near, settings.far, settings.distance);
                depthRange = this._camera.lastFar - this._camera.lastNear;
                texelWorld = (2 * radius) / map.size;
            }
            else {
                light.getWorldPosition(this._worldPos);
                const distance = light.distance;
                const far = settings.far > 0 ? settings.far : distance > 0 ? distance : 60;
                this._camera.fitSpot(this._worldPos, light.direction, light.angle, settings.near, far);
                depthRange = this._camera.lastFar - this._camera.lastNear;
     ...
     */
    renderAndSubmit(scene, camera, sceneRenderer, dt = 1 / 60) {
     ...
     */
    _collectBounds(scene, camera, sceneRenderer, dt, mapSize, stabilize) {
        const visible = sceneRenderer.collectVisible(scene, camera);
    ...
    _applyFit(target, radius, dt, mapSize, stabilize) {
        if (!this._hasFit) {
            this._hasFit = true;
            this._radius = radius;
            ...
            this._center.set(this._center.x + (target.x - this._center.x) * k, this._center.y + (target.y - this._center.y) * k, this._center.z + (target.z - this._center.z) * k);
        }
        if (!stabilize) {
        ...
        this._fitCenter.set(Math.round(this._center.x / grid) * grid, Math.round(this._center.y / grid) * grid, Math.round(this._center.z / grid) * grid);
    }
