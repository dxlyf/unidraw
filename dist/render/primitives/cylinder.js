...
export function cylinder(radiusTop = 0.5, radiusBottom = 0.5, height = 1, radialSegments = 32, heightSegments = 1, openEnded = false) {
    ...
    const addDisk = (y, radius, up) => {
        ...
        for (let col = 0; col < ws; col++) {
            ...
            if (up === 1)
                indices.push(center, ringStart + col, next);
            else
                indices.push(center, next, ringStart + col);
        }
    };
    if (!openEnded) {
...
/** 圆锥（顶点朝上 +Y） */
export function cone(radius = 0.5, height = 1, radialSegments = 32) {
