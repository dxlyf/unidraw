...
// ---------------------------------------------------------------------------
export function ensureOutwardWinding(data) {
    if (!data.indices)
        return;
    const idx = data.indices; // 内部由生成器提供可变数组
 ...
 */
export function lathe(rows, ws, positions, normals, uvs, indices) {
    const rings = rows.map(() => []);
    rows.forEach((row, ri) => {
