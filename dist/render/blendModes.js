...
export function blendState(color, alpha = { src: "one", dst: "one-minus-src-alpha" }) {
    return {
...
export function blendPreset(id) {
    return BLEND_PRESETS.find((p) => p.id === id) ?? BLEND_PRESETS[0];
}
/** 标准 alpha 混合状态（等价 `MaterialOptions.alphaBlend: true`） */
export const ALPHA_BLEND = blendState({ src: "src-alpha", dst: "one-minus-src-alpha" });
