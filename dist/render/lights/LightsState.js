...
const offsetOf = (name) => {
    ...
    if (!f)
        throw new Error(`[unidraw] LightsBlock 缺少字段 ${name}`);
    return f.offset / 4;
};
const OFFSET = {
...
export class LightsState {
     ...
     */
    addDirectional(direction, color, intensity = 1) {
     ...
     */
    addPoint(position, color, intensity = 1, distance = 0, decay = 2) {
     ...
     */
    addSpot(position, direction, color, intensity = 1, distance = 0, angle = Math.PI / 6, penumbra = 0.25) {
...
//# sourceMappingURL=LightsState.js.map