...
const EPS = 1e-5;
const vecClose = (a, b, eps = EPS) => Math.abs(a.x - b.x) < eps && Math.abs(a.y - b.y) < eps && Math.abs(a.z - b.z) < eps;
const quatClose = (a, b, eps = EPS) => Math.abs(a.x - b.x) < eps && Math.abs(a.y - b.y) < eps && Math.abs(a.z - b.z) < eps && Math.abs(a.w - b.w) < eps;
test("math: Euler → Quaternion → Euler 往返闭合（六种 order）", () => {
    ...
    for (const order of orders) {
        ...
        assert.ok(Math.abs(back.x - e.x) < 1e-4 && Math.abs(back.y - e.y) < 1e-4 && Math.abs(back.z - e.z) < 1e-4, `${order} 往返应闭合：(0.3,-0.7,1.1) → ${back.x},${back.y},${back.z}`);
    }
});
...
//# sourceMappingURL=math3d.test.js.map