...
import { AmbientLight, DirectionalLight, MAX_DIRECTIONAL_LIGHTS, MAX_POINT_LIGHTS, MAX_SPOT_LIGHTS, PointLight, SpotLight, LightsState, collectLights, } from "../render/lights/index.js";
/** 从打包数据里按 std140 偏移取 vec4（与 LightsState 布局一致） */
function vec4(data, offsetFloats) {
    return [data[offsetFloats], data[offsetFloats + 1], data[offsetFloats + 2], data[offsetFloats + 3]];
}
/** Float32 存储有精度损失，用容差比较 */
function nearArray(actual, expected, tol = 1e-6, label = "") {
    assert.equal(actual.length, expected.length, `${label} 长度不一致`);
    for (let i = 0; i < expected.length; i++) {
        assert.ok(Math.abs(actual[i] - expected[i]) <= tol, `${label} 第 ${i} 项应为 ${expected[i]}，实际 ${actual[i]}`);
    }
}
...
test("LightsState：超过上限返回 false 并计数（方向 4 / 点 8 / 聚 4）", () => {
    ...
    for (let i = 0; i < MAX_DIRECTIONAL_LIGHTS; i++)
        assert.equal(state.addDirectional(dir, white, 1), true);
    assert.equal(state.addDirectional(dir, white, 1), false);
    for (let i = 0; i < MAX_POINT_LIGHTS; i++)
        assert.equal(state.addPoint(new Vec3(i, 0, 0), white, 1, 0, 2), true);
    assert.equal(state.addPoint(new Vec3(0, 0, 0), white, 1, 0, 2), false);
    for (let i = 0; i < MAX_SPOT_LIGHTS; i++)
        assert.equal(state.addSpot(new Vec3(), dir, white, 1), true);
...
test("SceneRenderer.render：把收集到的灯光喂给材质（含层级里的点光）", () => {
    ...
    let receivedLights = null;
    const material = {
        beginFrame(_vp, _eye, lights) {
            if (lights)
                receivedLights = lights;
        },
        drawGeometry() { },
    };
...
//# sourceMappingURL=lights.test.js.map