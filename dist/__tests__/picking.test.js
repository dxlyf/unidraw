...
function addBox(device, scene, x, y = 0, z = 0, color = "#4c8dff") {
...
test("Raycaster：命中距离/命中点/未命中，以及背面剔除", () => {
    ...
    assert.ok(Math.abs(hit.distance - originDist) < 1e-3, `物体空间的 t 应等于到射线原点的距离：t=${hit.distance} originDist=${originDist}`);
...
test("ID 编解码：24 位编号往返一致", () => {
    ...
    for (const id of [1, 12345, 7000000]) {
        const [r, g, b] = encodeId(id);
        const round = (v) => Math.round((v / 255) * 255);
...
//# sourceMappingURL=picking.test.js.map