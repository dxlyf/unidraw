...
import { MAX_SHADOW_MAPS, SHADOW_BLOCK_BINDING, SHADOW_KIND_DIRECTIONAL, SHADOW_KIND_SPOT, SHADOW_SAMPLER_BINDING, SHADOW_TEXTURE_BINDING, ShadowState, } from "../render/shadow/index.js";
import { shadowResources } from "../render/shadow/ShadowResources.js";
import { TextureUsage } from "../gpu/types.js";
function buildScene(device, options = {}) {
...
test("ShadowSettings：默认值、clamp 与 Light.setShadow", () => {
    ...
    assert.equal(new ShadowSettings({ side: "随便" }).side, "back", "未知 side 回退");
...
test("ShadowCamera：方向光正交拟合（覆铜球 + 纹素对齐 + 深度递增）", () => {
    ...
    const right = new Vec3(direction.y * up.z - direction.z * up.y, direction.z * up.x - direction.x * up.z, direction.x * up.y - direction.y * up.x);
    ...
    const shifted = new ShadowCamera().fitDirectional(direction, new Vec3(direction.x * 0.01, direction.y * 0.01, direction.z * 0.01), radius, 512, 0, 0, 1.5);
...
test("材质绑定：受光材质声明阴影槽位，深度材质不声明", () => {
    ...
    assert.equal(layout.filter((e) => e.type === "texture").length, layout.filter((e) => e.type === "sampler").length, "WebGL2 的纹理/采样器按顺序配对，数量必须一致");
});
...
test("Mat4.ortho：ZO 约定（near → 0, far → 1）", () => {
    const m = Mat4.ortho(-1, 1, -1, 1, 1, 11);
    const z = (viewZ) => {
        const e = m.elements;
...
test("ShadowRenderer：bias 以世界单位换算、法线偏移按纹素自动、诊断字段可读", () => {
    ...
    const sun = scene.children.find((n) => n instanceof DirectionalLight);
