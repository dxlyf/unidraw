...
import { MAX_SHADOW_MAPS, SHADOW_BLOCK_BINDING, SHADOW_SAMPLER_BINDING, SHADOW_TEXTURE_BINDING, } from "../render/shadow/index.js";
function makeTexture(device, rgba) {
...
function uboFloats(material, binding) {
...
test("StandardMaterial：map / normalMap 绑定到 layout 的 binding 4/5", () => {
    ...
    assert.ok(layout.some((e) => e.binding === 15 && e.type === "sampler") &&
        layout.some((e) => e.binding === 16 && e.type === "sampler"), "两张纹理各配一个采样器条目");
    // WebGL2 后端把纹理/采样器按顺序配对：数量必须一致
    assert.equal(layout.filter((e) => e.type === "texture").length, layout.filter((e) => e.type === "sampler").length, "纹理与采样器数量必须一致");
    // 置空后回到占位纹理
    mat.setMap(null);
...
test("StandardMaterial：实例化管线可用（一次 draw + 实例矩阵流）", () => {
    ...
    assert.deepEqual(calls[0].vertexBuffers.map((v) => v.slot).sort(), [0, INSTANCE_VERTEX_SLOT], "标准顶点流 + 实例矩阵流");
    device.destroy();
});
//# sourceMappingURL=standard-material.test.js.map