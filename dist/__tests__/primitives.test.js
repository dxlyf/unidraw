...
function outsideRadius(data, radius, tol = 1e-4) {
...
function topology(data) {
    ...
    const key = (i) => `${q(pos[i * 3])},${q(pos[i * 3 + 1])},${q(pos[i * 3 + 2])}`;
    const edges = new Map();
    let degenerate = 0;
...
test("内置几何：闭合体积与解析值一致（同时校验绕序朝外）", () => {
    ...
    for (const c of cases) {
        ...
        assert.ok(err <= c.relTol, `${c.name} 体积应为 ${c.volume.toFixed(5)}（相对误差需 ≤ ${c.relTol}），实际 ${v.toFixed(5)}（误差 ${(err * 100).toFixed(2)}%）`);
    }
...
test("内置几何：顶点不超出声明尺寸，法线单位化", () => {
    ...
    const geos = [sphere(1, 24, 12), cylinder(0.7, 0.4, 1.4, 24), cone(0.6, 1.2, 24), torus(1, 0.3, 32, 16), capsule(0.4, 0.8, 24, 10)];
    for (const g of geos) {
