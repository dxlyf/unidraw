...
import { definePlugin } from "../app/Plugin.js";
...
function fakeCanvas(width = 256, height = 128) {
    return {
        width,
        height,
        clientWidth: width,
...
test("App：Mixer 与 Tween 随 step 推进，并驱动场景对象", async () => {
    ...
    mesh.material = new ColorMaterial(device, new Color(1, 1, 1, 1), { depth: false, targetFormat: "rgba8unorm" });
    app.scene.add(mesh);
    const clip = new AnimationClip("move", { duration: 1 }).addTrack(nodePositionTrack(mesh, vec3Keys([
        { time: 0, value: [0, 0, 0] },
        { time: 1, value: [2, 0, 0] },
    ])));
...
test("SceneRenderer.render：绘制前自动把相机喂给每个材质（否则画面全空）", () => {
    ...
    const other = { drawGeometry: () => drawCalls++ };
