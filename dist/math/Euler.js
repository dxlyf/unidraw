...
export class Euler {
    ...
    static DefaultOrder = DEFAULT_EULER_ORDER;
    constructor(x = 0, y = 0, z = 0, order = DEFAULT_EULER_ORDER) {
        this.x = x;
        this.y = y;
        this.z = z;
        this.order = order;
     ...
     */
    setFromQuaternion(q, order = this.order, update = true) {
        const x = q.x;
        const y = q.y;
        const z = q.z;
        const w = q.w;
        ...
        const wy = w * y2;
        const wz = w * z2;
        return this.setFromRotationElements(1 - (yy + zz), xy - wz, xz + wy, xy + wz, 1 - (xx + zz), yz - wx, xz - wy, yz + wx, 1 - (xx + yy), order, update);
    }
    /**
     ...
     */
    setFromRotationElements(m11, m12, m13, m21, m22, m23, m31, m32, m33, order, update) {
        switch (order) {
            case "XYZ":
     ...
     */
    equals(e, epsilon) {
        return (numEquals(this.x, e.x, epsilon) &&
