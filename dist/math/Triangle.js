 ...
 */
function closestPointOnSegment(a, b, point, target) {
    _v1.copy(b).sub(a);
...
export class Triangle {
    ...
    getNormal(target) {
        ...
        if (lengthSq > 0)
            return target.multiplyScalar(1 / Math.sqrt(lengthSq));
    ...
    getBarycoord(point, target) {
        ...
        if (!(denom > 1e-12 * dot00 * dot11))
            return target.set(NaN, NaN, NaN);
