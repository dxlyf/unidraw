 ...
 */
const SCALAR_SIZE = { f32: 4, i32: 4, u32: 4 };
export function typeSizeOf(type) {
    switch (type) {
        case "f32":
        case "i32":
        case "u32":
...
export function typeAlignOf(type) {
    switch (type) {
        case "f32":
        case "i32":
        case "u32":
...
export function std140Layout(fields) {
    let offset = 0;
    let blockAlign = 1;
    const out = { size: 0, align: 1, fields: [], byName: new Map() };
...
/** 数组第 i 个元素偏移 */
export function arrayElementOffset(field, index) {
    if (field.count <= 1)
        return field.offset;
    return field.offset + index * field.stride;
}
//# sourceMappingURL=std140.js.map