...
- `shadowColor` 支持**任意 CSS 颜色**：命名色（`"black"`/`"red"`）、`hsl()`、4/8 位 hex、
  `rgb()/rgba()` —— 交给浏览器自己规范化（`ctx.fillStyle` 读回），所以和原生完全一致；
  同一套解析也用于 `fillStyle`/`strokeStyle`（原来 `"black"` 这类写法会被解析失败，
  表现是「阴影根本不出现」或颜色画成白色）。解析结果按字符串缓存，热路径只做一次
  字符串比较；
- `fill()` / `stroke()` / `fillText()` 都会投影（原生也是）；`clearRect()` 不投影；
...
- 阴影按**参数组**（颜色 + 模糊 + 位移）分开：每组一张遮罩，并在「该组第一个 op」的
  ...
  三个坑值得记：合成位置早了会被本帧先画的背景 op 盖掉；**每组必须各留一份遮罩/结果
  纹理**（图层是当场 submit 的，合成却要等调用方 submit 才回放，共用一张会让所有合成
  读到最后一组的内容）；**每组的遮罩纹理 label 都一样，所以采样它的 pass 必须按纹理
  身份而不是 label 缓存 bind group**（否则第 2 组会拿到第 1 组的 bind group，
  blur/合成读的都是第 1 组的遮罩 → 阴影张冠李戴、甚至糊在别的图形上）；
- 每组的目标是**跨帧 LRU 缓存**（上限 4 组）：`shadowOffsetX`/`shadowBlur`/动画色这些
  参数一旦逐帧变化，每帧都是一个新组，不回收就是每帧新建一整套全屏 MSAA 目标 ——
  显存几秒内被吃光、帧率断崖式下跌。淘汰只针对「本帧没用到」的组（本帧用过的组合成
  还没回放，纹理不能销毁）；
...
- 代价：阴影开启时每帧多 1 趟几何 pass（遮罩）+ 2 趟全屏模糊 + 1 次全屏合成。
  实测（526×264、40 个图形的示例，解除帧率上限）无阴影 355fps → 开阴影 343fps
  （−3%）→ 再叠图层混合 306fps（−14%）；模糊是全屏开销，分辨率越高越明显。
...
- `stroke()` 的线段/接头是各自覆盖（**不支持**把整条描边当作一个区域只覆盖一次），
  半透明粗描边在接头处会比原生略深；
- 圆角 join 用扇形逼近；
- 字形纹理按**设备像素**栅格化（`pixelRatio` × CTM 缩放），高分屏/放大绘制不糊；
  但缩放比例在帧内变化时会各自缓存一份字形；
- **图层模式（11 种以目标为纹理的混合模式）成本与画布面积成正比**：一帧要多 3~4 趟
  全屏 MSAA pass（整帧搬进图层 → 源图层 → 混合 → 呈现），与图形数量无关。
  实测 1894×924 下，无混合 83~125fps、开混合 15~59fps（抖动大，量级可参考）。
  一次混合 op 的帧已在最后一次混合后立刻收摊（省一趟）；按 op 包围盒裁剪尚未实现 ——
  小图元上用混合模式目前仍按整块画布计算；
- `shadowColor` / `fillStyle` / `strokeStyle` 支持任意 CSS 颜色（命名色/`hsl()`/
  4·8 位 hex/`rgb()/rgba()`，交给浏览器规范化后缓存）；
