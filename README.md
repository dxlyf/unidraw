...
| 纹理 | 2D / **3D** / **2D 数组** / **cube**（`TextureDescriptor.dimension` + `depthOrArrayLayers`），逐层或整卷上传，**可逐层/逐面当渲染附件**（`RenderTarget({ dimension })` + `colorAttachment({ layer })`）；两个后端都支持，无头验证页 `examples/_verify-texture-dims` |
...
| 纹理回读 | `device.readTexturePixels(...)`：WebGL2 / WebGPU / Mock 三后端统一（左上原点、紧凑 RGBA）；`type: "float32"` 支持 `rgba32float` 与**深度纹理**（深度值在 R 通道），`layer` 支持**逐层/逐面回读**（cube 面、2D 数组层） |
| 离屏与后处理 | `RenderTarget`（格式/深度/**MSAA**/**分层（数组·cube）**/回读，三后端统一）+ `EffectComposer` 效果链（场景目标可选 MSAA → ping-pong 效果 → 呈现）；内置 `CopyPass`/`ToneMapPass`(ACES 等 4 种)/`BloomPass`/`VignettePass`/`GrayscalePass`/`ShaderPass`，自定义效果只要一对 fragment 源码 |
...
npm run build:verify   # 打包内部验证页（_verify-shared / _verify-sphere / _verify-2d-clip / _verify-2d-parity / _verify-texture-dims）
...
**分层渲染附件**，两个后端验证通过）、**浮点 / 深度回读**、2D 的 `clearRect` / 网页坐标系 /
pixelRatio / 阴影（真高斯 + spread）/ 25 种合成模式。

接下来（按价值排序，改法见 [docs/extension.md §7](docs/extension.md)）：

1. **点光源 cube map 阴影**：分层附件（`RenderTarget({ dimension: "cube" })` +
   `colorAttachment({ layer })`）与分层深度回读都已可用，剩下的是每面跑一趟深度 pass +
   在材质里按方向采样（球谐/PCF 部分可直接复用现有阴影代码）；
2. 计算管线 / 存储缓冲 / indirect draw（**WebGL2 无对应 API，只能 WebGPU-only + 降级**）；
3. 纹理压缩格式（BC/ETC2/ASTC）、WebGL2 `maxAnisotropy`、WebGPU `generateMipmaps`；
4. 2D 图层模式按 op 包围盒裁剪（当前成本与画布面积成正比）；
5. 命令编码去 GC、更激进的实例化（实例动画放进 vertex/compute）、WebGPU 原生队列级编码
