...
import type { BindGroup, Sampler, Texture } from "../device/resources.js";
import type { BindGroupLayoutEntryDescriptor } from "../device/descriptors.js";
 ...
 */
const STANDARD_EXTRA_ENTRIES: BindGroupLayoutEntryDescriptor[] = [
...
const STANDARD_FIELDS: UniformField[] = [
...
function resolveColor(value: Color | string | undefined, fallback: Color): Color {
  if (value === undefined) return fallback.clone();
  if (typeof value === "string") return new Color().setHex(value);
  return value.clone();
}

function applyColor(target: Color, value: Color | string): void {
  if (typeof value === "string") target.setHex(value);
...
function normalizeOptions(
  arg: Color | string | StandardMaterialOptions,
  extra: StandardMaterialOptions,
): StandardMaterialOptions {
  if (arg instanceof Color || typeof arg === "string") return { ...extra, color: arg };
  return arg;
}
...
export class StandardMaterial extends BaseMaterial {
  ...
  private readonly _emissive: Color;
  private _roughness: number;
  private _metalness: number;
  private _emissiveIntensity: number;
  private _normalScale: number;
  private _map: Texture | null = null;
  private _normalMap: Texture | null = null;
  /** 两张贴图共用的采样器（repeat + linear；缺省不启用 mipmap 过滤） */
  private readonly _sampler: Sampler;
  /** 未设置贴图时绑定的占位纹理（bind group 必须覆盖 layout 声明的全部 binding） */
  private readonly _whiteTexture: Texture;
  private readonly _flatNormalTexture: Texture;
  /** PBR 参数块（binding 17）：u_emissive + u_flags */
  private readonly standardBlock: UniformBlock;

  constructor(
    device: Device,
    colorOrOptions: Color | string | StandardMaterialOptions = {},
    ...
    this._whiteTexture = StandardMaterial.createPlaceholderTexture(
      device,
      label ? `${label}-white` : "standard-white",
      WHITE_PIXEL,
    );
    this._flatNormalTexture = StandardMaterial.createPlaceholderTexture(
      device,
      label ? `${label}-flat-normal` : "standard-flat-normal",
      FLAT_NORMAL_PIXEL,
    );
  ...
  private static createPlaceholderTexture(device: Device, label: string, pixel: Uint8Array): Texture {
