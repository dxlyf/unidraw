...
function collectEdges(polys: readonly Pt2[][]): Edge[] {
  const edges: Edge[] = [];
  for (const pts of polys) {
    ...
    for (let i = 0; i < n; i++) {
      const a = pts[i]!;
      const b = pts[(i + 1) % n]!;
      if (Math.abs(a[1] - b[1]) < EPS_Y) continue; // 水平边不参与求交
...
function hasSelfIntersection(pts: readonly Pt2[]): boolean {
  const n = pts.length;
  if (n < 4) return false;
  const seg = (i: number, j: number): [number, number, number, number] => [pts[i]![0], pts[i]![1], pts[j]![0], pts[j]![1]];
  const cross = (ox: number, oy: number, ax: number, ay: number, bx: number, by: number): number =>
    (ax - ox) * (by - oy) - (ay - oy) * (bx - ox);
  for (let i = 0; i < n; i++) {
    const [ax, ay, bx, by] = seg(i, (i + 1) % n);
    for (let j = i + 1; j < n; j++) {
      if (j === i || (j + 1) % n === i || j === (i + 1) % n) continue; // 相邻边共享端点
...
export function scanlineFill(polys: readonly Pt2[][], rule: FillRule): Pt2[][] {
  ...
  const bands: number[] = [];
  for (const y of ys) {
    if (bands.length === 0 || y - bands[bands.length - 1]! > EPS_Y) bands.push(y);
  }

  const out: Pt2[][] = [];
  const hits: { x: number; dir: number; edge: Edge }[] = [];
  for (let bi = 0; bi + 1 < bands.length; bi++) {
    const y0 = bands[bi]!;
    const y1 = bands[bi + 1]!;
    if (y1 - y0 < EPS_Y) continue;
    const mid = (y0 + y1) * 0.5;
    ...
    if (rule === "evenodd") {
      for (let i = 0; i + 1 < hits.length; i += 2) spans.push([i, i + 1]);
    } else {
      let winding = 0;
      let start = -1;
    ...
    for (const [li, ri] of spans) {
      ...
      const xl1 = xAt(le, y1);
      const xr1 = xAt(re, y1);
      if (xr0 - xl0 < EPS_X && xr1 - xl1 < EPS_X) continue;
      const p00: Pt2 = [xl0, y0];
      const p10: Pt2 = [xr0, y0];
      const p11: Pt2 = [xr1, y1];
      const p01: Pt2 = [xl1, y1];
...
function perContourTriangles(polys: readonly Pt2[][]): Pt2[][] {
  const out: Pt2[][] = [];
  for (const pts of polys) {
    if (pts.length < 3) continue;
    const tris = triangulateSimplePolygon(pts as Pt2[]);
    if (!tris) continue;
 ...
 */
export function fillTriangles(polys: readonly Pt2[][], rule: FillRule): Pt2[][] {
  ...
  if (total > MAX_SCANLINE_EDGES) return perContourTriangles(polys);
