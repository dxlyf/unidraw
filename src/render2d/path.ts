/**
 * Path2D：与 Canvas2D 类似的路径描述 + 曲线压平。
 *
 * 路径在“用户空间”记录；fill()/stroke() 时按容差压平为折线轮廓。
 * 曲线支持：quadratic / bezier / arc / arcTo / ellipse / roundRect / rect。
 */
...
function curveSteps(flatTolerance: number): number {
  // 容差越小采样越多；至少 4 段
