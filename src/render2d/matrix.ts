...
export function copyAffine(m: Affine): Affine {
  return { a: m.a, b: m.b, c: m.c, d: m.d, e: m.e, f: m.f };
}

export function multiplyAffine(m: Affine, t: Affine, out: Affine): Affine {
...
export function transformPoint(m: Affine, x: number, y: number, out: { x: number; y: number }): { x: number; y: number } {
...
export function untransformRect(m: Affine, x: number, y: number, w: number, h: number): { x: number; y: number; w: number; h: number } {
  const det = m.a * m.d - m.b * m.c;
  if (Math.abs(det) < 1e-12) return { x: 0, y: 0, w: 0, h: 0 };
  const ia = m.d / det;
  const ib = -m.b / det;
  const ic = -m.c / det;
  const id = m.a / det;
