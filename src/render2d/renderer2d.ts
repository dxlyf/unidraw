/**
 * Canvas2D —— 框架级 2D 绘图器（WebGL2 / WebGPU 共用）。
 *
 * 能力：矩形/圆角矩形/直线/二次与三次贝塞尔/arc/arcTo/椭圆等路径、
 * 填充(凸/凹多边形)与描边(线宽/线帽/线连接/miter)、纯色与线性渐变样式、
 * 全局透明度、仿射变换与 save/restore、字形栅格化文本、矩形裁剪。
 *
 * 每帧把图元 CPU 侧三角化，flush 时按 op 顺序以 scissor+drawIndexed 提交。
 */
...
import { triangulateSimplePolygon, polygonArea2 } from "./triangulate.js";
import type { Affine, Pt2 } from "./matrix.js";
...
const DEFAULT_FONT = "28px system-ui, 'Segoe UI', 'PingFang SC', 'Microsoft YaHei', sans-serif";
...
function normalOffset(ax: number, ay: number, bx: number, by: number, hw: number): { x: number; y: number } {
  const dx = bx - ax;
  const dy = by - ay;
...
function lineIntersect(ax: number, ay: number, dx1: number, dy1: number, bx: number, by: number, dx2: number, dy2: number): { x: number; y: number } | null {
...
function intersectRects(a: DeviceRect, b: DeviceRect): DeviceRect {
...
function sameClip(a: DeviceRect | null, b: DeviceRect | null): boolean {
  if (a === null || b === null) return a === b;
  return a.x === b.x && a.y === b.y && a.w === b.w && a.h === b.h;
}

function detectRectContour(path: Path2D): [number, number, number, number] | null {
