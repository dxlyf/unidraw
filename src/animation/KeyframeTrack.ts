...
export class KeyframeTrack<T> {
  readonly id: number = nextTrackId++;
  ...
  private readonly _after: (() => void) | undefined;
  private readonly _scratch: T;
  private readonly _blend: T;
  private readonly _prev: T;
  private _hasPrev = false;
  /** 顺序播放时的段游标缓存（避免每次二分） */
  private _cursor = 0;

  constructor(options: KeyframeTrackOptions<T>) {
    if (options.keys.length === 0) throw new Error("[unidraw] KeyframeTrack 至少需要一个关键帧");
  ...
  sample(time: number, out?: T): T {
    ...
    if (time <= keys[0]!.time) return this.type.copy(target, keys[0]!.value);
    if (time >= keys[last]!.time) return this.type.copy(target, keys[last]!.value);

    // 顺序播放时游标通常只需 +1；否则退化为向前扫描（关键帧数量级很小）
    let i = this._cursor;
    if (i >= last) i = 0;
    while (i < last && keys[i + 1]!.time <= time) i++;
