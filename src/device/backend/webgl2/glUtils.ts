...
import { BufferUsage, TextureFormat } from "../../../gpu/types.js";
...
export function bufferTarget(usage: number): number {
...
export function bufferUsageHint(usage: number): number {
...
/** 纹理格式 → GL 常量（有类型的安全映射） */
export function textureGLParams(gl: GL, format: TextureFormat): { internal: number; format: number; type: number } {
  switch (format) {
    case "r8unorm":
...
export function attributeGLType(gl: GL, glType: "FLOAT" | "UNSIGNED_BYTE" | "BYTE" | "UNSIGNED_SHORT" | "SHORT"): number {
  switch (glType) {
    case "FLOAT":
      return gl.FLOAT;
    case "UNSIGNED_BYTE":
...
export function describeRenderer(gl: GL): string {
  try {
