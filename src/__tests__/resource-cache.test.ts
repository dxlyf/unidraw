...
import type { RenderPipelineDescriptor } from "../device/descriptors.js";
...
function descriptor(program: ReturnType<MockDevice["createProgram"]>): RenderPipelineDescriptor {
  return {
    program,
    bindGroupLayouts: [],
...
test("缓存键：源码/状态相同 → 键相同，不同 → 键不同", () => {
  ...
  assert.notEqual(
    pipelineCacheKey({ ...base, primitive: { ...base.primitive!, cullMode: "front" } }, 1),
    key,
    "剔除模式不同 → 不同管线",
  );
  assert.notEqual(
    pipelineCacheKey({ ...base, depthStencil: { ...base.depthStencil!, depthCompare: "always" } }, 1),
    key,
    "深度比较不同 → 不同管线",
  );
...
test("动态偏移按值内联：逐 draw 复用数组、环形槽正确", () => {
  ...
  for (const mesh of meshes) shared.draw(pass, mesh);
  pass.end();
  ...
  shared.beginFrame(camera.viewProjection, camera.eyePosition);
  for (const mesh of meshes) shared.draw(p, mesh);
  p.end();
