...
function makeImage(width: number, height: number, bgra = false): Uint8Array {
...
test("readTexturePixels：整图回读为紧凑 RGBA（左上原点）", async () => {
  ...
  const at = (x: number, y: number) => Array.from(pixels.subarray((y * 32 + x) * 4, (y * 32 + x) * 4 + 4));
...
test("readTexturePixels：子区域以左上角为原点", async () => {
  ...
  const at = (x: number, y: number) => Array.from(sub.subarray((y * 3 + x) * 4, (y * 3 + x) * 4 + 4));
  // 对应原图 (4,5) 与 (6,6)
...
async function expectError(fn: () => unknown, pattern: RegExp): Promise<void> {
