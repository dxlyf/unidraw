...
import { UnlitColorMaterial, PhongMaterial, ColorMaterial } from "../render/material.js";
import { Geometry } from "../render/Geometry.js";
import { Mesh } from "../render/Mesh.js";
import { Color } from "../math/color.js";

/** 统计顶点/三角形/绕序-法线一致率 */
function stats(data: GeometryData): { verts: number; tris: number; outwardPct: number } {
  ...
  for (let i = 0; i < idx.length; i += 3) {
    ...
    tot++;
    if (((aby * acz - abz * acy) * nx + (abz * acx - abx * acz) * ny + (abx * acy - aby * acx) * nz) / (gl * nl) > 0) ok++;
  }
  return { verts: p.length / 3, tris: idx.length / 3, outwardPct: tot ? ok / tot : 1 };
}

function checkShape(data: GeometryData, name: string, minVerts = 12): void {
  const s = stats(data);
...
function makeScene(device: MockDevice, material: ColorMaterial | UnlitColorMaterial | PhongMaterial): void {
