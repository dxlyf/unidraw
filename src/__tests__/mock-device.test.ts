...
import { MockDevice, MOCK_CANVAS_FORMAT } from "../device/backend/mock/MockDevice.js";
...
test("mock：全流程绘制一条 drawIndexed", () => {
  ...
  const calls = (device as MockDevice).drawCalls;
...
test("mock：geometry 上传与索引类型选择", () => {
  ...
  const huge: { positions: number[]; normals: number[]; uvs: number[]; indices: number[] } = {
    positions: [],
    normals: [],
    uvs: [],
    indices: [],
