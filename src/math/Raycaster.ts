...
export class Raycaster {
  ...
  private readonly _invWorld = new Mat4();
  private readonly _localOrigin = new Vec3();
  private readonly _localDir = new Vec3();
  private readonly _localRay = new Ray();
  private readonly _dir = new Vec3();
  private readonly _near = new Vec3();
  private readonly _farPt = new Vec3();
  private readonly _v0 = new Vec3();
  private readonly _v1 = new Vec3();
  private readonly _v2 = new Vec3();
  private readonly _w0 = new Vec3();
  private readonly _w1 = new Vec3();
  private readonly _w2 = new Vec3();
  private readonly _normal = new Vec3();
  private _hits: Intersection[] = [];
  ...
  constructor(options: RaycasterOptions = {}) {
    ...
    if (options.backfaceCulling !== undefined) this.backfaceCulling = options.backfaceCulling;
    if (options.firstHitOnly !== undefined) this.firstHitOnly = options.firstHitOnly;
  }
  ...
  intersectObject(mesh: Mesh, recursive = true, out: Intersection[] = []): Intersection[] {
    const visit = (node: Node3D) => {
      if (!(node instanceof Mesh)) return;
      const hit = this._intersectMesh(node);
      if (hit) out.push(hit);
    };
    if (recursive) mesh.traverseVisible(visit);
    else visit(mesh);
  ...
  private _intersectMesh(mesh: Mesh): Intersection | null {
    const geo = mesh.geometry;
    if (!geo.positionsCPU) return null; // 未保留 CPU 数据 → 只能用颜色拾取
    ...
    // 包围球预剔除
    if (this._localRay.intersectSphere(geo.boundingSphereCenter, geo.boundingSphereRadius) === null) return null;
...
/** out = (b-a) × (c-a) */
function crossABAC(a: Vec3, b: Vec3, c: Vec3, out: Vec3): Vec3 {
  const abx = b.x - a.x;
  const aby = b.y - a.y;
  const abz = b.z - a.z;
  const acx = c.x - a.x;
