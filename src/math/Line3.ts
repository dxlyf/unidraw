...
export class Line3 {
  ...
  setFromPoints(points: Vec3[]): this {
    ...
    if (first) this.start.copy(first);
    if (second) this.end.copy(second);
    return this;
  }
  ...
  getCenter(target = new Vec3()): Vec3 {
    return target.set(
   ...
   */
  at(t: number, target = new Vec3()): Vec3 {
    return target.set(
   ...
   * 即「最近点就是重合的那个端点」。
   */
  closestPointToPointParameter(point: Vec3, clampToLine = true): number {
  ...
  closestPointToPoint(point: Vec3, clampToLine = true, target = new Vec3()): Vec3 {
