...
export class Frustum {
  ...
  readonly planes: Vec4[] = [new Vec4(), new Vec4(), new Vec4(), new Vec4(), new Vec4(), new Vec4()];

  setFromProjectionMatrix(m: Mat4, zZeroToOne = true): this {
    const e = m.elements;
    // 行向量（列主序矩阵）
    const r0x = e[0]!;
    const r0y = e[4]!;
    ...
    if (zZeroToOne) set(4, r2x, r2y, r2z, r2w); // near: z >= 0
    else set(4, r3x + r2x, r3y + r2y, r3z + r2z, r3w + r2w);
  ...
  intersectsSphere(centerOrSphere: Vec3 | Sphere, radius?: number): boolean {
    const center = radius === undefined ? (centerOrSphere as Sphere).center : (centerOrSphere as Vec3);
    const r = radius === undefined ? (centerOrSphere as Sphere).radius : radius;
    for (let i = 0; i < 6; i++) {
  ...
  containsPoint(point: Vec3): boolean {
    for (let i = 0; i < 6; i++) {
      const p = this.planes[i]!;
      if (p.x * point.x + p.y * point.y + p.z * point.z + p.w < 0) return false;
    }
    return true;
  }
  ...
  clone(): Frustum {
    ...
    for (let i = 0; i < 6; i++) out.planes[i]!.copy(this.planes[i]!);
    return out;
  }
